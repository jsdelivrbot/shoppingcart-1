/**
 * TranZform Eligibility - Business Services
 * The YAML consists fo the following Service definitions.    1. Get Member Eligibility.
 *
 * OpenAPI spec version: 5.00.004.004
 *
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */
import * as models from './models';
export interface AdditionalInsurance {
    /**
     * The group ID should hold the value AdditionalInsurance
     */
    groupId?: AdditionalInsurance.GroupIdEnum;
    attributeDetails?: models.AttributeDetails;
}
export declare namespace AdditionalInsurance {
    enum GroupIdEnum {
        AdditionalInsurance,
    }
}
